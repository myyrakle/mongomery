...
실행 모드는 3개로 분리되어 있고, 명령줄에서 명시해서 실행합니다.

1. `verify`: 연결 체크 + 버전 확인
2. `schema`: 컬렉션 목록/옵션/인덱스만 복제
3. `copy`: `_id` 커서 기반 문서 복제 + 중단 지점 재개

`copy`는 스키마 단계가 완료되어 있는 대상 DB를 기준으로 동작합니다. 독립 실행 시 `copy` 이전에 `schema` 또는 `run`을 먼저 실행하세요.

`run` 또는 기본 실행은 `verify → schema → copy`를 순서대로 한 번에 수행합니다.
...
```bash
go run . --config ./config.json                # 기본: run(전체)
